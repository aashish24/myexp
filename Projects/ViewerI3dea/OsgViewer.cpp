...
osg::Node* loadData( const std::string& file )
{
...
osg::Node* loadData( const boost::filesystem::path& path )
{
  if( boost::filesystem::is_directory( path ) )
  {
    ...
    try
    {
      for( itr; itr != endItr; ++itr )
      {
        ...
        {
          std::cout << "Reading file: " << itr->path().string() << std::endl;
          osg::ref_ptr< osg::Node > node = loadData( itr->path().string() );
          if( node.valid() )
          {
  ...
  {
    return loadData( path.string() );
  }
}

// Look for data in a specific folder set by environment 
// variable OSG_DATA_PATH
osg::Node* loadDataEnvVarOsgDataPath()
{
  const char* path = getenv( "OSG_DATA_PATH" );
  
  if( !path )
  {
    std::cout << "Environment variable OSG_DATA_PATH not set: " << std::endl;
    return 0x00;
  }

  boost::filesystem::path bPath( path, boost::filesystem::native ); 
  if( !bPath.empty() )
  {
    return loadData( bPath );
  }
  else
  {
    std::cout << "Error reading data from: " << bPath.string() << std::endl;
    return 0x00;
  }
}
...
int main(int argc, char** argv)
{
  ...
  {
    std::cout << "No data read from command line arguments: " << std::endl;
    std::cout << "Loading data from OSG_DATA_PATH environment variable: " << std::endl;
    loadedModel = loadDataEnvVarOsgDataPath();
    
    if( !loadedModel.valid() )
    {
      std::cerr << "Failed to load data." << std::endl;
      std::cerr << "Please check your OSG_DATA_PATH environment variable." << std::endl;
    }
  }
