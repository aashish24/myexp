...
#define MAINOHF             0x4D4D
#define EDITOHF             0x3D3D  // this is the start of the editor config
...
//------------------------------
// max length of objects' names
#define MAX_OBJNAME_LEN    255
//------------------------------

//----------------------
// name OHF gives to dummy objects
#define DEFAULT_DUMMY "$$$DUMMY"
//------------------------
...
LMap emptyMap = {0, "", 1, 1, 0, 0, 0, 0};  //added initialization of tiling to 0 to avoid warning
...
PrimType LPrimitive::GetType(){
	return m_type;
}

//-------------------------------------------------------
// LDummy implementation
//-------------------------------------------------------
LDummy::LDummy() 
:   LPrimitive()
{
...
uint LImporter::GetDummyCount(){
	return m_dummies.size();
}
    
LDummy& LImporter::GetDummy(uint index){
	return m_dummies[index];
}

int LImporter::FindMeshIndex(const std::string& name)
{
    for (uint i=0; i<m_meshes.size(); i++)
        if (m_meshes[i].IsObject(name))
            return i;
...
uint LImporter::GetPrimitiveCount(){
	return m_tree.size();
}

LHierarchyInfo& LImporter::GetHierarchyInfo(uint index){
	return m_tree[index];
}
...
bool LOHF::Load(std::istream &is)
{
    ...
    if (m_buffer == 0)
    {
        fprintf(stderr, "LOHF::LoadFile - not enough memory (malloc failed)");
        return false;
    }
    ...
    if(is.gcount() != m_bufferSize)
    {
        free(m_buffer);
        m_bufferSize = 0;
        fprintf(stderr, "LOHF::LoadFile - error reading from stream");
        return false;
    }
...
bool LOHF::Load(const char *filename)
{
    FILE *f;
    f = fopen(filename, "rb");
    if (f == 0)
    {
        fprintf(stderr, "LOHF::LoadFile - cannot open file");
        return false;
    }
    fseek(f, 0, SEEK_END);
    ...
    if (fread(m_buffer, m_bufferSize, 1, f) != 1)
    {
        ...
        free(m_buffer);
        m_bufferSize = 0;
        fprintf(stderr, "LOHF::LoadFile - error reading from file");
        return false;
    }
    fclose(f);
...
int LOHF::ReadASCIIZ(char *buf, int max_count)
{
    int count;
...
void LOHF::Seek(int offset, int origin)
{
...
bool LOHF::FindChunk(LChunk &target, const LChunk &parent)
{
    if (Pos() >= parent.end)
        return false;
...
void LOHF::SkipChunk(const LChunk &chunk)
{
    Seek(chunk.end, SEEK_START);
}

void LOHF::GotoChunk(const LChunk &chunk)
{
    Seek(chunk.start, SEEK_START);
}

LColor3 LOHF::ReadColor(const LChunk &chunk)
{
    LColor3 col = black;
    GotoChunk(chunk);
    switch (chunk.id)
    ...
    case LIN_COLOR_24:
        ...
        break;
    default:
        fprintf(stderr, "LOHF::ReadColor - error this is not a color chunk");
    }
    return col;
}

float LOHF::ReadPercentage(const LChunk &chunk)
{
    GotoChunk(chunk);
    switch (chunk.id)
    {
    ...
    fprintf(stderr, "LOHF::ReadPercentage - error, the chunk is not a percentage chunk");
    return 0;
}
...
bool LOHF::ReadOHF()
{
    ...
    edit.id = EDITOHF;
    mainchunk = ReadChunk();
    if (mainchunk.id != MAINOHF)
    {
        fprintf(stderr, "LOHF::ReadOHF - wrong file format");
        return false;
    }
...
void LOHF::ReadLight(const LChunk &parent)
{
    float t;
    LVector3 v;
    LLight light;
...
void LOHF::ReadCamera(const LChunk &parent)
{
    LVector3 v,t;
    LCamera camera;
...
void LOHF::ReadMesh(const LChunk &parent)
{
...
void LOHF::ReadFaceList(const LChunk &chunk, LMesh &mesh)
{
    // variables
    ...
    {
        fprintf(stderr, "LOHF::ReadFaceList - internal error: wrong chunk passed as parameter");
        return;
    }
    GotoChunk(chunk);
...
void LOHF::ReadMaterial(const LChunk &parent)
{
    // variables
    LChunk chunk;
    LChunk child;
    ...
    while (chunk.end <= parent.end)
    {
        ...
        case MAT_REFLMAP:
            ...
            {
                fprintf(stderr, "LOHF::ReadMaterial - error, expected chunk not found");
                return;
            }
            ReadASCIIZ(str, 30);
            if (strcmp(str, "") == 0)
...
void LOHF::ReadMap(const LChunk &chunk, LMap& map)
{
    LChunk child;
    char str[20];
    GotoChunk(chunk);
...
int LOHF::ReadDummy(const LChunk& parent){
	LChunk dummy_hdr;
    dummy_hdr.id = DUMMY_NAME;
    GotoChunk(parent);
    if (!FindChunk(dummy_hdr, parent))
    	return -1;
    GotoChunk(dummy_hdr);
    ReadASCIIZ(m_objName, MAX_OBJNAME_LEN);
    //fprintf(stdout, "I got a DUMMY: %s\n", str);
    
    LDummy dummy;
    dummy.SetName(m_objName);
    m_dummies.push_back(dummy); 
    return m_dummies.size()-1;
}

void LOHF::ReadKeyframeData(const LChunk &parent)
{
    uint frames = 0;
    ...
    if (!FindChunk(node_hdr, parent))
        return;
    GotoChunk(node_hdr);
    ReadASCIIZ(str, MAX_OBJNAME_LEN);
  
    
    //read object's hierarchy    
    LHierarchyInfo hinfo;
    Seek(node_hdr.start + strlen(str) + 5, SEEK_START);    
    hinfo.m_hierarchy = ReadShort();
    
    
    //see if it's a dummy
    if (!strcmp(str, DEFAULT_DUMMY)){
    	int dummy_index = ReadDummy(parent);
    	if ( dummy_index >= 0){
    		hinfo.index = dummy_index;
    		hinfo.ptype = PRIM_HELPER;    		
    		m_tree.push_back(hinfo);   		
    	}   	
    	return;   	
    }
    ...
    //see if it's a mesh
    int mesh_index = FindMeshIndex(str);    
    if (mesh_index == -1) 
    	return;
    	
    LMesh mesh;
    mesh = GetMesh(mesh_index);      
    hinfo.index = mesh_index;
    hinfo.ptype = PRIM_GEOMETRY;
    m_tree.push_back(hinfo);
    //fprintf(stdout, " Mesh %s, hierarchy is: %i, type is %i\n", str, hh, mesh->GetType());
    ...
    if (FindChunk(pivotchunk, parent))
    {
        ...
        /*pivot.z =*/ ReadFloat();
        float px, py, pz;
        px=ReadFloat(); py=ReadFloat(); pz=ReadFloat(); 
        //fprintf (stdout,"Object %s has pivot point: %f, %f, %f\n", str,px,py,pz );
    }
    GotoChunk(parent);
