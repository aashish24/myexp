...
float calculateGaussianWeight(float standardDeviation, float distance)
{
  ...
  float sampleDepth = texture2D( depths, vec2( vTexCoord.x, vTexCoord.y -  distance * blurSize ) ).z;

  vec3 normal = texture2D( normals, vTexCoord ).xyz;
  vec3 sampleNormal = texture2D( normals, vec2( vTexCoord.x, vTexCoord.y -  distance * blurSize ) ).xyz;
