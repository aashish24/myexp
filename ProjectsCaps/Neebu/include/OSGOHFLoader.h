...
#ifndef _OSGOHFLOADER_H_
#define _OSGOHFLOADER_H_
...
//--------------------------------------------
// TODO:
// - Read groups and dummies
//--------------------------------------------

//-------------------------------------------
//Object hierarchies: The root object is given the number -1 ( FFFF )
const short int MAINOHF_ROOT = -1;
...
// LChunk, LTri internal, but as they are used in a
...
//-------------------------------------------------
// LPrimitive class
// represents a OHF scene primitive (objects, shapes, lights, dummies, etc)
// (for now I'm using it for geometry and dummies only)
enum PrimType{
	PRIM_GEOMETRY,
	PRIM_HELPER
};

class LPrimitive : public LObject {
public:	
	virtual ~LPrimitive();
	PrimType GetType();
protected:
...
//---------------------------------------------
class LDummy:  public LPrimitive {
public:
	//constructor, initializes m_type
	LDummy();
	virtual ~LDummy();	
};
...
//-----------------------------------------------------
// Structure to store scene's hierarchy info from the keyframer chunk
struct LHierarchyInfo{
	PrimType ptype;
	unsigned int index;  //refers to the index in meshes or dummies vector
	short int m_hierarchy;
};


//------------------------------------------------

class LMesh : public LPrimitive
{
public:
...
public:
    ...
    // returns number of dummies in scene
    uint GetDummyCount();
    // returns reference to dummy
    LDummy& GetDummy(uint index);
    // returns index of mesh with a given name, -1 if not found
    int FindMeshIndex(const std::string &name);
    uint GetPrimitiveCount();
    LHierarchyInfo& GetHierarchyInfo(uint index);
...
protected:
    ...
    //dummy objects
    std::vector<LDummy> m_dummies;    
    //the scene hierarchy from the keyframer chunk
    std::vector<LHierarchyInfo> m_tree;
    
};
//------------------------------------------------

class LOHF : public LImporter
{
public:
    // the default contructor
    LOHF();
    // constructs the object and loads the file
    LOHF(const char *filename);
    // destructor
    virtual ~LOHF();
    // load 3ds file
...
protected:
    ...
    //reads a dummy chunk
    int  ReadDummy(const LChunk &parent);
};
